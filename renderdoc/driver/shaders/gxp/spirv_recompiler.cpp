...
//#include <3rdparty/glslang/SPIRV/disassemble.h>
#include "shader/spirv_glsl.hpp"
...
struct VertexProgramOutputProperties
{
  ...
  VertexProgramOutputProperties() : name(nullptr), component_count(0), location(0) {}

  VertexProgramOutputProperties(const char *name, std::uint32_t component_count,
                                std::uint32_t location)
      : name(name), component_count(component_count), location(location)
  {
  }
};
using VertexProgramOutputPropertiesMap =
    std::map<SceGxmVertexProgramOutputs, VertexProgramOutputProperties>;

// ******************************
// * Functions (implementation) *
// ******************************

static spv::Id create_array_if_needed(spv::Builder &b, const spv::Id param_id, const Input &input,
                                      const uint32_t explicit_array_size = 0)
{
  // disabled
  return param_id;
...
static spv::Id get_param_type(spv::Builder &b, const Input &input)
{
  ...
  {
    case GenericType::SCALER: return get_type_scalar(b, input);
    case GenericType::VECTOR: return get_type_vector(b, input);
    case GenericType::ARRAY: return get_type_array(b, input);
    default: return get_type_fallback(b);
  }
}
...
spv::StorageClass reg_type_to_spv_storage_class(usse::RegisterBank reg_type)
{
  switch(reg_type)
  {
    case usse::RegisterBank::TEMP: return spv::StorageClassPrivate;
    case usse::RegisterBank::PRIMATTR: return spv::StorageClassInput;
    case usse::RegisterBank::OUTPUT: return spv::StorageClassOutput;
    case usse::RegisterBank::SECATTR: return spv::StorageClassUniformConstant;
    case usse::RegisterBank::FPINTERNAL: return spv::StorageClassPrivate;
...
static spv::Id create_param_sampler(spv::Builder &b, const std::string &name)
{
  ...
  spv::Id image_type =
      b.makeImageType(sampled_type, spv::Dim2D, false, false, false, 1, spv::ImageFormatUnknown);
...
static spv::Id create_input_variable(spv::Builder &b, SpirvShaderParameters &parameters,
                                     utils::SpirvUtilFunctions &utils, const FeatureState &features,
                                     const char *name, const RegisterBank bank,
                                     const std::uint32_t offset, spv::Id type,
                                     const std::uint32_t size, spv::Id force_id = spv::NoResult,
                                     DataType dtype = DataType::F32)
{
  std::uint32_t total_var_comp = size / 4;
  spv::Id var =
      !force_id ? (b.createVariable(reg_type_to_spv_storage_class(bank), type, name)) : force_id;
  Operand dest;
  dest.bank = bank;
  dest.num = offset;
  dest.type = dtype;
  ...
  auto get_dest_mask = [&]() {
    ...
    {
      case 1: dest_mask = 0b1; break;

      case 2: dest_mask = 0b11; break;

      case 3: dest_mask = 0b111; break;
...
static spv::Id create_builtin_sampler(spv::Builder &b, const FeatureState &features,
                                      TranslationState &translation_state, const std::string &name)
{
...
static void create_fragment_inputs(spv::Builder &b, SpirvShaderParameters &parameters,
                                   utils::SpirvUtilFunctions &utils, const FeatureState &features,
                                   TranslationState &translation_state,
                                   NonDependentTextureQueryCallInfos &tex_query_infos,
                                   SamplerMap &samplers, const SceGxmProgram &program)
{
  static const std::unordered_map<std::uint32_t, std::pair<std::string, std::uint32_t>> name_map = {
      {0xD000, {"v_Position", 0}},   {0xC000, {"v_Fog", 3}},        {0xA000, {"v_Color0", 1}},
      {0xB000, {"v_Color1", 2}},     {0x0, {"v_TexCoord0", 4}},     {0x1000, {"v_TexCoord1", 5}},
      {0x2000, {"v_TexCoord2", 6}},  {0x3000, {"v_TexCoord3", 7}},  {0x4000, {"v_TexCoord4", 8}},
      {0x5000, {"v_TexCoord5", 9}},  {0x6000, {"v_TexCoord6", 10}}, {0x7000, {"v_TexCoord7", 11}},
      {0x8000, {"v_TexCoord8", 12}}, {0x9000, {"v_TexCoord9", 13}},
  };
  ...
  const SceGxmProgramAttributeDescriptor *descriptor =
      reinterpret_cast<const SceGxmProgramAttributeDescriptor *>(
          reinterpret_cast<const std::uint8_t *>(&vertex_varyings_ptr->vertex_outputs1) +
          vertex_varyings_ptr->vertex_outputs1);
  ...
  for(size_t i = 0; i < vertex_varyings_ptr->varyings_count; i++, descriptor++)
  {
    ...
    if((descriptor->attribute_info & 0x4000F000) != 0xF000)
    {
      ...
      // Reason is for compability between vertex and fragment. This is like an anti-crash when
      // linking. Fragment will only copy what it needed.
  ...
  for(auto &query_info : tex_query_infos)
  {
    if(coords[query_info.coord_index].first == spv::NoResult)
    {
      ...
      coords[query_info.coord_index].first = b.createVariable(
          spv::StorageClassInput, b.makeVectorType(b.makeFloatType(32), /*tex_coord_comp_count*/ 4),
          coord_name.c_str());

      b.addDecoration(coords[query_info.coord_index].first, spv::DecorationLocation,
                      TEXCOORD_BASE_LOCATION + query_info.coord_index);
  ...
  if(program.is_native_color())
  {
    ...
    if(features.direct_fragcolor)
    {
      ...
      spv::Id last_frag_data =
          b.createOp(spv::OpAccessChain, v4, {last_frag_data_arr, b.makeIntConstant(0)});
    ...
    else if(features.support_shader_interlock || features.support_texture_barrier)
    {
      ...
      auto color_attachment =
          create_builtin_sampler(b, features, translation_state, "f_colorAttachment");
      ...
      current_coord =
          b.createUnaryOp(spv::OpConvertFToS, b.makeVectorType(i32, 4), b.createLoad(current_coord));
      current_coord = b.createOp(spv::OpVectorShuffle, b.makeVectorType(i32, 2),
                                 {current_coord, current_coord, 0, 1});
    ...
    if(source != spv::NoResult)
    {
      ...
      target_to_store.type =
          std::get<0>(shader::get_parameter_type_store_and_name(program.get_fragment_output_type()));
 ...
 */
static size_t calculate_variable_size(const SceGxmProgramParameter &parameter,
                                      const DataType store_type)
{
...
static void copy_uniform_block_to_register(spv::Builder &builder, spv::Id sa_bank, spv::Id block,
                                           spv::Id ite, const int start, const int vec4_count)
{
  utils::make_for_loop(
      builder, ite, builder.makeIntConstant(0), builder.makeIntConstant(vec4_count), [&]() {
        spv::Id to_copy = builder.createAccessChain(
            spv::StorageClassUniform, block, {builder.makeIntConstant(0), builder.createLoad(ite)});

        spv::Id dest = builder.createAccessChain(
            spv::StorageClassPrivate, sa_bank,
            {builder.createBinOp(spv::OpIAdd, builder.getTypeId(builder.createLoad(ite)),
                                 builder.createLoad(ite), builder.makeIntConstant(start))});

        builder.createStore(builder.createLoad(to_copy), dest);
      });
}

static spv::Id create_uniform_block(spv::Builder &b, const FeatureState &features,
                                    const int base_binding, const int size_vec4_granularity,
                                    const bool is_vert)
{
...
static SpirvShaderParameters create_parameters(
    spv::Builder &b, const SceGxmProgram &program, utils::SpirvUtilFunctions &utils,
    const FeatureState &features, TranslationState &translation_state,
    SceGxmProgramType program_type, NonDependentTextureQueryCallInfos &texture_queries,
    const std::vector<SceGxmVertexAttribute> *hint_attributes)
{
  ...
  for(const auto &buffer : program_input.uniform_buffers)
  {
    if(buffer.reg_block_size > 0)
    {
      ...
      copy_uniform_block_to_register(b, spv_params.uniforms, spv_buffer, ite_copy,
                                     buffer.reg_start_offset / 4, reg_block_size_in_f32v);
    }
  }
  ...
  for(const auto [index, size] : buffer_sizes)
  {
    ...
    utils::make_for_loop(b, ite_copy, b.makeIntConstant(0), b.makeIntConstant(size), [&]() {
      spv::Id to_copy = b.createAccessChain(spv::StorageClassUniform, buffer,
                                            {b.makeIntConstant(0), b.createLoad(ite_copy)});
      spv::Id offset_base =
          b.createBinOp(spv::OpIMul, i32_type, b.makeIntConstant(4), b.createLoad(ite_copy));
      for(int i = 0; i < 4; ++i)
      {
        spv::Id offset = b.createBinOp(spv::OpIAdd, i32_type, offset_base, b.makeIntConstant(i));
        spv::Id src = b.createBinOp(spv::OpVectorExtractDynamic, f32_type, b.createLoad(to_copy),
                                    b.makeIntConstant(i));
        spv::Id dest = b.createAccessChain(
            spv::StorageClassPrivate, spv_params.memory,
            {b.createBinOp(spv::OpIAdd, i32_type, offset, b.makeIntConstant(last_base))});
  ...
  const auto add_var_to_reg = [&](const Input &input, const std::string &name,
                                  std::uint16_t semantic, bool pa, std::int32_t location) {
    const spv::Id param_type = get_param_type(b, input);
    int type_size = (int)get_data_type_size(input.type);
  ...
  for(size_t i = 0; i < program.parameter_count; ++i)
  {
       ...
       parameter.category == SCE_GXM_PARAMETER_CATEGORY_ATTRIBUTE)
    {
      ...
      const auto [store_type, param_type_name] =
          shader::get_parameter_type_store_and_name(parameter_type);
  ...
  {
    std::visit(overloaded{[&](const LiteralInputSource &s) {
                          ...
                          [&](const UniformBufferInputSource &s) {
                            ...
                            utils::store(b, spv_params, utils, features, reg,
                                         b.makeIntConstant(base), 0b1, 0);
                          },
                          ...
                          [&](const AttributeInputSource &s) {
                            ...
                            in_fcount_allocated +=
                                ((input.array_size * input.component_count + 3) / 4 * 4);
                          }},
               input.source);
  }
  ...
  if((in_fcount_allocated == 0) && (program.primary_reg_count != 0))
  {
    ...
    if(hint_attributes)
    {
      ...
      for(std::size_t i = 0; i < hint_attributes->size(); i++)
      {
        ...
        add_var_to_reg(inp, std::string("attribute") + std::to_string(i), 0, true,
  ...
  if(literal_pairs.size() != 0)
  {
    ...
    auto create_new_literal_pack = [&]() {
      ...
      {
        composite_var = b.makeCompositeConstant(
            b.makeVectorType(f32_type, static_cast<int>(constituents.size())), constituents);
      }
      else
      {
        composite_var = constituents[0];
      }
      create_input_variable(b, spv_params, utils, features, nullptr, RegisterBank::SECATTR,
                            composite_base, spv::NoResult,
  ...
  if(program_type == SceGxmProgramType::SCE_GXM_VERTEX_PROGRAM)
  {
    spv::Id f32 = b.makeFloatType(32);
    spv::Id v4 = b.makeVectorType(f32, 4);
  ...
  {
    create_fragment_inputs(b, spv_params, utils, features, translation_state, texture_queries,
                           samplers, program);
  }
...
static void generate_shader_body(spv::Builder &b, const SpirvShaderParameters &parameters,
                                 const SceGxmProgram &program, const FeatureState &features,
                                 utils::SpirvUtilFunctions &utils, spv::Function *end_hook_func,
                                 const NonDependentTextureQueryCallInfos &texture_queries)
{
  // Do texture queries
  usse::convert_gxp_usse_to_spirv(b, program, features, parameters, utils, end_hook_func,
                                  texture_queries);
}

static spv::Function *make_frag_finalize_function(spv::Builder &b,
                                                  const SpirvShaderParameters &parameters,
                                                  const SceGxmProgram &program,
                                                  utils::SpirvUtilFunctions &utils,
                                                  const FeatureState &features,
                                                  TranslationState &translate_state)
{
  ...
  spv::Function *frag_fin_func = b.makeFunctionEntry(
      spv::NoPrecision, b.makeVoidType(), "frag_output_finalize", {}, decorations, &frag_fin_block);
  ...
  if(program.is_native_color() && features.should_use_shader_interlock())
  {
    ...
    spv::Id depth =
        b.createOp(spv::OpAccessChain, b.makeFloatType(32), {coord_id, b.makeIntConstant(2)});
    spv::Id translated_id =
        b.createUnaryOp(spv::OpConvertFToS, b.makeVectorType(signed_i32, 4), coord_id);
    translated_id = b.createOp(spv::OpVectorShuffle, b.makeVectorType(signed_i32, 2),
                               {translated_id, translated_id, 0, 1});
    b.createNoResultOp(spv::OpImageWrite,
                       {b.createLoad(translate_state.color_attachment_id), translated_id, color});
  }
  else
  {
    spv::Id out = b.createVariable(spv::StorageClassOutput,
                                   b.makeVectorType(b.makeFloatType(32), 4), "out_color");
...
static spv::Function *make_vert_finalize_function(spv::Builder &b,
                                                  const SpirvShaderParameters &parameters,
                                                  const SceGxmProgram &program,
                                                  utils::SpirvUtilFunctions &utils,
                                                  const FeatureState &features,
                                                  TranslationState &translation_state)
{
  ...
  spv::Function *vert_fin_func = b.makeFunctionEntry(
      spv::NoPrecision, b.makeVoidType(), "vert_output_finalize", {}, decorations, &vert_fin_block);
  ...
  const auto add_vertex_output_info = [&](SceGxmVertexProgramOutputs vo, const char *name,
                                          std::uint32_t component_count, std::uint32_t location) {
  ...
  add_vertex_output_info(SCE_GXM_VERTEX_PROGRAM_OUTPUT_TEXCOORD0, "v_TexCoord0",
                         calculate_copy_comp_count(coord_infos[0]), 4);
  add_vertex_output_info(SCE_GXM_VERTEX_PROGRAM_OUTPUT_TEXCOORD1, "v_TexCoord1",
                         calculate_copy_comp_count(coord_infos[1]), 5);
  add_vertex_output_info(SCE_GXM_VERTEX_PROGRAM_OUTPUT_TEXCOORD2, "v_TexCoord2",
                         calculate_copy_comp_count(coord_infos[2]), 6);
  add_vertex_output_info(SCE_GXM_VERTEX_PROGRAM_OUTPUT_TEXCOORD3, "v_TexCoord3",
                         calculate_copy_comp_count(coord_infos[3]), 7);
  add_vertex_output_info(SCE_GXM_VERTEX_PROGRAM_OUTPUT_TEXCOORD4, "v_TexCoord4",
                         calculate_copy_comp_count(coord_infos[4]), 8);
  add_vertex_output_info(SCE_GXM_VERTEX_PROGRAM_OUTPUT_TEXCOORD5, "v_TexCoord5",
                         calculate_copy_comp_count(coord_infos[5]), 9);
  add_vertex_output_info(SCE_GXM_VERTEX_PROGRAM_OUTPUT_TEXCOORD6, "v_TexCoord6",
                         calculate_copy_comp_count(coord_infos[6]), 10);
  add_vertex_output_info(SCE_GXM_VERTEX_PROGRAM_OUTPUT_TEXCOORD7, "v_TexCoord7",
                         calculate_copy_comp_count(coord_infos[7]), 11);
  add_vertex_output_info(SCE_GXM_VERTEX_PROGRAM_OUTPUT_TEXCOORD8, "v_TexCoord8",
                         calculate_copy_comp_count(coord_infos[8]), 12);
  add_vertex_output_info(SCE_GXM_VERTEX_PROGRAM_OUTPUT_TEXCOORD9, "v_TexCoord9",
                         calculate_copy_comp_count(coord_infos[9]), 13);
  ...
  for(const auto vo : vertex_outputs_list)
  {
    if(vertex_outputs & vo)
    {
      ...
      if(vo == SCE_GXM_VERTEX_PROGRAM_OUTPUT_POSITION)
      {
        ...
        spv::Id viewport_id = b.createAccessChain(
            spv::StorageClassUniform, translation_state.render_info_id, {b.makeIntConstant(1)});
        spv::Id screen_width_id = b.createAccessChain(
            spv::StorageClassUniform, translation_state.render_info_id, {b.makeIntConstant(2)});
        spv::Id screen_height_id = b.createAccessChain(
            spv::StorageClassUniform, translation_state.render_info_id, {b.makeIntConstant(3)});

        spv::Id pred =
            b.createOp(spv::OpFOrdLessThan, b.makeBoolType(), {b.createLoad(viewport_id), half});
        ...
        spv::Id height_recp =
            b.createBinOp(spv::OpFDiv, f32, neg_two, b.createLoad(screen_height_id));
...
static SpirvCode convert_gxp_to_spirv_impl(
    const SceGxmProgram &program, const std::string &shader_hash, const FeatureState &features,
    TranslationState &translation_state, const std::vector<SceGxmVertexAttribute> *hint_attributes,
    bool force_shader_debug,
    std::function<bool(const std::string &ext, const std::string &dump)> dumper)
{
  ...
  // Lock/unlock and read texel for shader interlock. Texture barrier will have glTextureBarrier()
  // called so we don't have to worry too much. Texture barrier will not be accurate and may be
  // broken though.
  if(program_type == SceGxmProgramType::SCE_GXM_FRAGMENT_PROGRAM)
  {
  ...
  // Generate parameters
  SpirvShaderParameters parameters = create_parameters(
      b, program, utils, features, translation_state, program_type, texture_queries, hint_attributes);

  if(program.is_fragment())
  {
    end_hook_func =
        make_frag_finalize_function(b, parameters, program, utils, features, translation_state);
  }
  else
  {
    end_hook_func =
        make_vert_finalize_function(b, parameters, program, utils, features, translation_state);
  }

  for(auto &var_to_reg : translation_state.var_to_regs)
  {
    create_input_variable(b, parameters, utils, features, "",
                          var_to_reg.pa ? RegisterBank::PRIMATTR : RegisterBank::SECATTR,
                          var_to_reg.offset, spv::NoResult, var_to_reg.size, var_to_reg.var,
                          var_to_reg.dtype);
  }
  ...
  if(program.is_vertex())
  {
    ...
    utils::make_for_loop(b, ite, b.makeIntConstant(0), b.makeIntConstant(REG_O_COUNT / 4), [&]() {
      Operand target_to_store;
      spv::Id dest =
          b.createAccessChain(spv::StorageClassPrivate, parameters.outs, {b.createLoad(ite)});
...
static std::string convert_spirv_to_glsl(const std::string &shader_name, SpirvCode spirv_binary,
                                         const FeatureState &features,
                                         TranslationState &translation_state)
{
...
// ***************************
usse::SpirvCode convert_gxp_to_spirv(
    const SceGxmProgram &program, const std::string &shader_name, const FeatureState &features,
    const std::vector<SceGxmVertexAttribute> *hint_attributes, bool maskupdate,
    bool force_shader_debug,
    std::function<bool(const std::string &ext, const std::string &dump)> dumper)
{
  TranslationState translation_state;
  ...
  return convert_gxp_to_spirv_impl(program, shader_name, features, translation_state,
                                   hint_attributes, force_shader_debug, dumper);
}

std::string convert_gxp_to_glsl(
    const SceGxmProgram &program, const std::string &shader_name, const FeatureState &features,
    const std::vector<SceGxmVertexAttribute> *hint_attributes, bool maskupdate,
    bool force_shader_debug,
    std::function<bool(const std::string &ext, const std::string &dump)> dumper)
{
  ...
  translation_state.is_fragment = program.is_fragment();
  translation_state.is_maskupdate = maskupdate;
  std::vector<uint32_t> spirv_binary = convert_gxp_to_spirv_impl(program, shader_name, features,
  translation_state, hint_attributes, force_shader_debug, dumper);
