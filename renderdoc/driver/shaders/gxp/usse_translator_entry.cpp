...
template <typename V>
std::optional<const USSEMatcher<V>> DecodeUSSE(uint64_t instruction) {
...
spv::Function *USSERecompiler::get_or_recompile_block(const usse::USSEBlock &block) {
    ...
    const auto sub_name = std::string("block_") + std::string(visitor.is_translating_secondary_program() ? "sec" : "prim") + "_" + std::to_string(block.offset);
