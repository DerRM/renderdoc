...
bool USSETranslatorVisitor::vmad(ExtVecPredicate pred, Imm1 skipinv, Imm1 gpi1_swiz_ext,
                                 Imm1 opcode2, Imm1 dest_use_bank_ext, Imm1 end, Imm1 src1_bank_ext,
                                 RepeatMode repeat_mode, Imm1 gpi0_abs, RepeatCount repeat_count,
                                 bool nosched, Imm4 write_mask, Imm1 src1_neg, Imm1 src1_abs,
                                 Imm1 gpi1_neg, Imm1 gpi1_abs, Imm1 gpi0_swiz_ext, Imm2 dest_bank,
                                 Imm2 src1_bank, Imm2 gpi0_n, Imm6 dest_n, Imm4 gpi0_swiz,
                                 Imm4 gpi1_swiz, Imm2 gpi1_n, Imm1 gpi0_neg, Imm1 src1_swiz_ext,
                                 Imm4 src1_swiz, Imm6 src1_n)
{
  std::stringstream format;
  format << "0x" << std::setfill('0') << std::setw(16) << std::hex << m_instr;
  std::string disasm_str = format.str() + ": " + disasm::e_predicate_str(ext_vec_predicate_to_ext(pred)) + "VMAD";
  ...
  inst.opr.src1 =
      decode_src12(inst.opr.src1, src1_n, src1_bank, src1_bank_ext, true, 7, m_second_program);
  inst.opr.dest =
      decode_dest(inst.opr.dest, dest_n, dest_bank, dest_use_bank_ext, true, 7, m_second_program);
  ...
  RDCLOG("%s %s %s %s %s", disasm_str.c_str(),
         disasm::operand_to_str(inst.opr.dest, write_mask).c_str(),
         disasm::operand_to_str(inst.opr.src0, write_mask, src0_repeat_offset).c_str(),
         disasm::operand_to_str(inst.opr.src1, write_mask, src1_repeat_offset).c_str(),
         disasm::operand_to_str(inst.opr.src2, write_mask, src2_repeat_offset).c_str());
  ...
  {
    RDCERR("Source not loaded (vsrc0: %" PRIu32 ", vsrc1: %" PRIu32 ", vsrc2: %" PRIu32 ")",
              vsrc0, vsrc1, vsrc2);
    return false;
  }
...
bool USSETranslatorVisitor::vmad2(Imm1 dat_fmt, Imm2 pred, Imm1 skipinv, Imm1 src0_swiz_bits2,
                                  Imm1 syncstart, Imm1 src0_abs, Imm1 src1_bank_ext,
                                  Imm1 src2_bank_ext, Imm3 src2_swiz, Imm1 src1_swiz_bit2,
                                  Imm1 nosched, Imm4 dest_mask, Imm2 src1_mod, Imm2 src2_mod,
                                  Imm1 src0_bank, Imm2 dest_bank, Imm2 src1_bank, Imm2 src2_bank,
                                  Imm6 dest_n, Imm2 src1_swiz_bits01, Imm2 src0_swiz_bits01,
                                  Imm6 src0_n, Imm6 src1_n, Imm6 src2_n)
{
  Instruction inst;
  ...
  inst.opr.src1 =
      decode_src12(inst.opr.src1, src1_n, src1_bank, src1_bank_ext, true, 7, m_second_program);
  inst.opr.src2 =
      decode_src12(inst.opr.src2, src2_n, src2_bank, src2_bank_ext, true, 7, m_second_program);
  ...
  const Swizzle4 tb_decode_src0_swiz[] = {
      SWIZZLE_CHANNEL_4(X, X, X, X), SWIZZLE_CHANNEL_4(Y, Y, Y, Y), SWIZZLE_CHANNEL_4(Z, Z, Z, Z),
      SWIZZLE_CHANNEL_4(W, W, W, W), SWIZZLE_CHANNEL_4(X, Y, Z, W), SWIZZLE_CHANNEL_4(Y, Z, X, W),
      SWIZZLE_CHANNEL_4(X, Y, W, W), SWIZZLE_CHANNEL_4(Z, W, X, Y)};

  const Swizzle4 tb_decode_src1_swiz[] = {
      SWIZZLE_CHANNEL_4(X, X, X, X), SWIZZLE_CHANNEL_4(Y, Y, Y, Y), SWIZZLE_CHANNEL_4(Z, Z, Z, Z),
      SWIZZLE_CHANNEL_4(W, W, W, W), SWIZZLE_CHANNEL_4(X, Y, Z, W), SWIZZLE_CHANNEL_4(X, Y, Y, Z),
      SWIZZLE_CHANNEL_4(Y, Y, W, W), SWIZZLE_CHANNEL_4(W, Y, Z, W)};

  const Swizzle4 tb_decode_src2_swiz[] = {
      SWIZZLE_CHANNEL_4(X, X, X, X), SWIZZLE_CHANNEL_4(Y, Y, Y, Y), SWIZZLE_CHANNEL_4(Z, Z, Z, Z),
      SWIZZLE_CHANNEL_4(W, W, W, W), SWIZZLE_CHANNEL_4(X, Y, Z, W), SWIZZLE_CHANNEL_4(X, Z, W, W),
      SWIZZLE_CHANNEL_4(X, X, Y, Z), SWIZZLE_CHANNEL_4(X, Y, Z, Z)};
  ...
  format << "0x" << std::setfill('0') << std::setw(16) << std::hex << m_instr;
  // Log the instruction
  RDCLOG("%s: %s%s2 %s %s %s %s", format.str().c_str(),
         disasm::e_predicate_str(static_cast<ExtPredicate>(pred)), disasm::opcode_str(op),
         disasm::operand_to_str(inst.opr.dest, dest_mask).c_str(),
         disasm::operand_to_str(inst.opr.src0, dest_mask).c_str(),
         disasm::operand_to_str(inst.opr.src1, dest_mask).c_str(),
         disasm::operand_to_str(inst.opr.src2, dest_mask).c_str());
...
bool USSETranslatorVisitor::vdp(ExtVecPredicate pred, Imm1 skipinv, bool clip_plane_enable,
                                Imm1 opcode2, Imm1 dest_use_bank_ext, Imm1 end, Imm1 src1_bank_ext,
                                RepeatMode repeat_mode, Imm1 gpi0_abs, RepeatCount repeat_count,
                                bool nosched, Imm4 write_mask, Imm1 src1_neg, Imm1 src1_abs,
                                Imm3 clip_plane_n, Imm2 dest_bank, Imm2 src1_bank, Imm2 gpi0_n,
                                Imm6 dest_n, Imm4 gpi0_swiz, Imm3 src1_swiz_w, Imm3 src1_swiz_z,
                                Imm3 src1_swiz_y, Imm3 src1_swiz_x, Imm6 src1_n)
{
  Instruction inst;
  ...
  const SwizzleChannel tb_swizz_dec[] = {
      SwizzleChannel::_X, SwizzleChannel::_Y, SwizzleChannel::_Z,
      SwizzleChannel::_W, SwizzleChannel::_0, SwizzleChannel::_1,
      SwizzleChannel::_2, SwizzleChannel::_H, SwizzleChannel::_UNDEFINED};
  ...
  std::stringstream format;
  format << "0x" << std::setfill('0') << std::setw(16) << std::hex << m_instr;
  LOG_DISASM("%s: %sVDP %s %s %s", format.str().c_str(),
   disasm::e_predicate_str(ext_vec_predicate_to_ext(pred)), disasm::operand_to_str(inst.opr.dest,
   write_mask, 0).c_str(),
      disasm::operand_to_str(inst.opr.src1, src_mask, src1_repeat_offset).c_str(),
      disasm::operand_to_str(inst.opr.src2, src_mask, src2_repeat_offset).c_str());
  ...
  {
    RDCERR("Source not loaded (lhs: %" PRIu32 ", rhs: %" PRIu32 ")", lhs, rhs);
    return false;
  }
...
spv::Id USSETranslatorVisitor::do_alu_op(Instruction &inst, const Imm4 source_mask)
{
  ...
  switch(inst.opcode)
  {
    ...
    case Opcode::VDSX:
    case Opcode::VF16DSX: result = m_b.createOp(spv::OpDPdx, source_type, ids); break;

    case Opcode::VDSY:
    case Opcode::VF16DSY: result = m_b.createOp(spv::OpDPdy, source_type, ids); break;
    ...
    case Opcode::VF16FRC:
    {
      ...
      else
      {
        ...
        spv::Id source2_floored =
            m_b.createBuiltinCall(source_type, std_builtins, GLSLstd450Floor, {vsrc2});
    ...
    default:
    {
      RDCERR("Unimplemented ALU opcode instruction: %s", disasm::opcode_str(inst.opcode).c_str());
      break;
    }
  }
...
bool USSETranslatorVisitor::v32nmad(ExtVecPredicate pred, bool skipinv, Imm2 src1_swiz_10_11,
                                    bool syncstart, Imm1 dest_bank_ext, Imm1 src1_swiz_9,
                                    Imm1 src1_bank_ext, Imm1 src2_bank_ext, Imm4 src2_swiz,
                                    bool nosched, Imm4 dest_mask, Imm2 src1_mod, Imm1 src2_mod,
                                    Imm2 src1_swiz_7_8, Imm2 dest_bank_sel, Imm2 src1_bank_sel,
                                    Imm2 src2_bank_sel, Imm6 dest_n, Imm7 src1_swiz_0_6, Imm3 op2,
                                    Imm6 src1_n, Imm6 src2_n)
{
  Instruction inst;

  static const Opcode tb_decode_vop_f32[] = {
      Opcode::VMUL, Opcode::VADD, Opcode::VFRC, Opcode::VDSX,
      Opcode::VDSY, Opcode::VMIN, Opcode::VMAX, Opcode::VDP,
  };
  static const Opcode tb_decode_vop_f16[] = {
      Opcode::VF16MUL, Opcode::VF16ADD, Opcode::VF16FRC, Opcode::VF16DSX,
      Opcode::VF16DSY, Opcode::VF16MIN, Opcode::VF16MAX, Opcode::VF16DP,
  };
  Opcode opcode;
  ...
  inst.opr.dest =
      decode_dest(inst.opr.dest, dest_n, dest_bank_sel, dest_bank_ext, true, 7, m_second_program);
  inst.opr.src1 =
      decode_src12(inst.opr.src1, src1_n, src1_bank_sel, src1_bank_ext, true, 7, m_second_program);
  inst.opr.src1.flags = decode_modifier(src1_mod);
  inst.opr.src1.index = 1;

  inst.opr.src2 =
      decode_src12(inst.opr.src2, src2_n, src2_bank_sel, src2_bank_ext, true, 7, m_second_program);
  ...
  const auto src1_swizzle_enc =
      src1_swiz_0_6 | src1_swiz_7_8 << 7 | src1_swiz_9 << 9 | src1_swiz_10_11 << 10;
  ...
  std::stringstream format;
  format << "0x" << std::setfill('0') << std::setw(16) << std::hex << m_instr;
  RDCLOG("%s: %s%s %s %s %s", format.str().c_str(), disasm::e_predicate_str(pred_translated),
             disasm::opcode_str(opcode).c_str(),
             disasm::operand_to_str(inst.opr.dest, dest_mask).c_str(),
             disasm::operand_to_str(inst.opr.src1, source_mask).c_str(),
             disasm::operand_to_str(inst.opr.src2, source_mask).c_str());
...
bool USSETranslatorVisitor::v16nmad(ExtVecPredicate pred, bool skipinv, Imm2 src1_swiz_10_11,
                                    bool syncstart, Imm1 dest_bank_ext, Imm1 src1_swiz_9,
                                    Imm1 src1_bank_ext, Imm1 src2_bank_ext, Imm4 src2_swiz,
                                    bool nosched, Imm4 dest_mask, Imm2 src1_mod, Imm1 src2_mod,
                                    Imm2 src1_swiz_7_8, Imm2 dest_bank_sel, Imm2 src1_bank_sel,
...
{
  // TODO
  return v32nmad(pred, skipinv, src1_swiz_10_11, syncstart, dest_bank_ext, src1_swiz_9,
                 src1_bank_ext, src2_bank_ext, src2_swiz, nosched, dest_mask, src1_mod, src2_mod,
                 src1_swiz_7_8, dest_bank_sel, src1_bank_sel, src2_bank_sel, dest_n, src1_swiz_0_6,
                 op2, src1_n, src2_n);
}

bool USSETranslatorVisitor::vcomp(ExtPredicate pred, bool skipinv, Imm2 dest_type, bool syncstart,
                                  bool dest_bank_ext, bool end, bool src1_bank_ext,
                                  RepeatCount repeat_count, bool nosched, Imm2 op2, Imm2 src_type,
                                  Imm2 src1_mod, Imm2 src_comp, Imm2 dest_bank, Imm2 src1_bank,
                                  Imm7 dest_n, Imm7 src1_n, Imm4 write_mask)
{
  Instruction inst;

  // All of them needs to be doubled
  inst.opr.dest =
      decode_dest(inst.opr.dest, dest_n, dest_bank, dest_bank_ext, true, 8, m_second_program);
  inst.opr.src1 =
      decode_src12(inst.opr.src1, src1_n, src1_bank, src1_bank_ext, true, 8, m_second_program);
  inst.opr.src1.flags = decode_modifier(src1_mod);
  ...
  static const Opcode tb_decode_vop[] = {Opcode::VRCP, Opcode::VRSQ, Opcode::VLOG, Opcode::VEXP};

  static const DataType tb_decode_src_type[] = {DataType::F32, DataType::F16, DataType::C10,
                                                DataType::UNK};

  static const DataType tb_decode_dest_type[] = {DataType::F32, DataType::F16, DataType::C10,
                                                 DataType::UNK};
  ...
  switch(op)
  {
    ...
    case Opcode::VRSQ:
    {
      ...
      result = m_b.createBuiltinCall(m_b.getTypeId(result), std_builtins, GLSLstd450InverseSqrt,
                                     {result});
      break;
    }
  ...
  std::stringstream format;
  format << "0x" << std::setfill('0') << std::setw(16) << std::hex << m_instr;
  RDCLOG("%s: %s%s %s %s", format.str().c_str(), disasm::e_predicate_str(pred),
             disasm::opcode_str(op).c_str(),
             disasm::operand_to_str(inst.opr.dest, write_mask, dest_repeat_offset).c_str(),
             disasm::operand_to_str(inst.opr.src1, src_mask, src1_repeat_offset).c_str());
  END_REPEAT()
...
bool USSETranslatorVisitor::sop(Imm2 pred, Imm1 cmod1, Imm1 skipinv, Imm1 nosched, Imm2 asel1,
                                Imm1 dest_bank_ext, Imm1 end, Imm1 src1_bank_ext,
                                Imm1 src2_bank_ext, Imm1 cmod2, Imm3 count, Imm1 amod1, Imm2 asel2,
                                Imm3 csel1, Imm3 csel2, Imm1 amod2, Imm2 dest_bank, Imm2 src1_bank,
                                Imm2 src2_bank, Imm7 dest_n, Imm1 src1_mod, Imm2 cop, Imm2 aop,
                                Imm1 asrc1_mod, Imm1 dest_mod, Imm7 src1_n, Imm7 src2_n)
{
  static auto selector_zero = [](spv::Builder &b, const spv::Id type, const spv::Id src1_color,
                                 const spv::Id src2_color, const spv::Id src1_alpha,
                                 const spv::Id src2_alpha) {
    return utils::make_uniform_vector_from_type(b, type, 0);
  };

  static auto selector_src1_color = [](spv::Builder &b, const spv::Id type, const spv::Id src1_color,
                                       const spv::Id src2_color, const spv::Id src1_alpha,
                                       const spv::Id src2_alpha) { return src1_color; };

  static auto selector_src2_color = [](spv::Builder &b, const spv::Id type, const spv::Id src1_color,
                                       const spv::Id src2_color, const spv::Id src1_alpha,
                                       const spv::Id src2_alpha) { return src2_color; };

  static auto selector_src1_alpha = [](spv::Builder &b, const spv::Id type,
                                       const spv::Id src1_color, const spv::Id src2_color,
                                       const spv::Id src1_alpha, const spv::Id src2_alpha) {
  ...
  static auto selector_src2_alpha = [](spv::Builder &b, const spv::Id type,
                                       const spv::Id src1_color, const spv::Id src2_color,
                                       const spv::Id src1_alpha, const spv::Id src2_alpha) {
    if(!b.isScalarType(type) || b.getNumTypeComponents(type) > 1)
    {
  ...
  static Opcode operations[] = {Opcode::FADD, Opcode::FSUB, Opcode::FMIN, Opcode::FMAX};

  using SelectorFunc = std::function<spv::Id(spv::Builder &, const spv::Id, const spv::Id,
                                             const spv::Id, const spv::Id, const spv::Id)>;
  ...
  static SelectorFunc alpha_selector_1[] = {
      selector_zero, selector_src1_alpha, selector_src2_alpha,
  ...
  inst.opr.src1 =
      decode_src12(inst.opr.src1, src1_n, src1_bank, src1_bank_ext, false, 7, m_second_program);
  inst.opr.src2 =
      decode_src12(inst.opr.src2, src2_n, src2_bank, src2_bank_ext, false, 7, m_second_program);
  inst.opr.dest =
      decode_dest(inst.opr.dest, dest_n, dest_bank, dest_bank_ext, false, 7, m_second_program);
  ...
  {
    RDCERR("Invalid color opcode: %" PRIi32, (int)cop);
    return true;
  }

  if(aop >= sizeof(operations) / sizeof(Opcode))
  {
    RDCERR("Invalid alpha opcode: %" PRIi32, (int)aop);
    return true;
  }
  ...
  {
    RDCERR("Unknown color/alpha selector (csel1: %" PRIu8 ", csel2: %" PRIu8 ", asel1: %" PRIu8
           ", asel2: %" PRIu8,
           csel1, csel2, asel1, asel2);
    return true;
  }
  ...
  if(!color_selector_1_func || !color_selector_2_func || !alpha_selector_1_func ||
     !alpha_selector_2_func)
  {
    RDCERR("Unimplemented color/alpha selector (csel1: %" PRIu8 ", csel2: %" PRIu8
           ", asel1: %" PRIu8 ", asel2: %" PRIu8,
           csel1, csel2, asel1, asel2);
    return true;
  }
  ...
  auto apply_opcode = [&](Opcode op, spv::Id type, spv::Id lhs, spv::Id rhs) {
    switch(op)
    {
      case Opcode::FADD: { return m_b.createBinOp(spv::OpFAdd, type, lhs, rhs);
      }

      case Opcode::FSUB: { return m_b.createBinOp(spv::OpFSub, type, lhs, rhs);
      }
      ...
      default:
      {
        RDCERR("Unsuppported sop opcode: %s", disasm::opcode_str(op).c_str());
        break;
      }
    }
  ...
  spv::Id factored_rgb_lhs =
      color_selector_1_func(m_b, src_color_type, src1_color, src2_color, src1_alpha, src2_alpha);
  spv::Id factored_rgb_rhs =
      color_selector_2_func(m_b, src_color_type, src1_color, src2_color, src1_alpha, src2_alpha);

  spv::Id factored_a_lhs =
      alpha_selector_1_func(m_b, src_alpha_type, src1_color, src2_color, src1_alpha, src2_alpha);
  spv::Id factored_a_rhs =
      alpha_selector_2_func(m_b, src_alpha_type, src1_color, src2_color, src1_alpha, src2_alpha);
  ...
  std::stringstream format;
  format << "0x" << std::setfill('0') << std::setw(16) << std::hex << m_instr;
  RDCLOG("%s: {}", format.str().c_str(), disasm::opcode_str(color_op).c_str());
  RDCLOG("%s: {}", format.str().c_str(), disasm::opcode_str(alpha_op).c_str());
...
{
  RDCLOG("Unimplmenet Opcode: sop2");
  return true;
}

bool shader::usse::USSETranslatorVisitor::sop3()
{
  RDCLOG("Unimplmenet Opcode: sop3");
  return true;
}
  ...
  switch(count)
  {
    case 1:
      switch(config)
      {
        ...
        case 1:
          return std::optional<DualSrcLayout>({DualSrcId::INTERAL0, DualSrcId::NONE, DualSrcId::NONE});
        case 2:
          return std::optional<DualSrcLayout>(
              {DualSrcId::INTERNAL1, DualSrcId::NONE, DualSrcId::NONE});
        case 3:
          return std::optional<DualSrcLayout>({DualSrcId::INTERAL2, DualSrcId::NONE, DualSrcId::NONE});
        default: return {};
      }
    case 2:
      switch(config)
      {
        case 0:
          return std::optional<DualSrcLayout>(
              {DualSrcId::UNIFIED, DualSrcId::INTERNAL1, DualSrcId::NONE});
        case 1:
          return std::optional<DualSrcLayout>(
              {DualSrcId::INTERAL0, DualSrcId::UNIFIED, DualSrcId::NONE});
        case 2:
          return std::optional<DualSrcLayout>(
              {DualSrcId::INTERAL0, DualSrcId::INTERNAL1, DualSrcId::NONE});
        default: return {};
      }
    case 3:
      switch(config)
      {
        case 0:
          return std::optional<DualSrcLayout>(
              {DualSrcId::UNIFIED, DualSrcId::INTERNAL1, DualSrcId::INTERAL2});
        case 1:
          return std::optional<DualSrcLayout>(
              {DualSrcId::INTERAL0, DualSrcId::UNIFIED, DualSrcId::INTERAL2});
        case 2:
          return std::optional<DualSrcLayout>(
              {DualSrcId::INTERAL0, DualSrcId::INTERNAL1, DualSrcId::UNIFIED});
        case 3:
          return std::optional<DualSrcLayout>(
              {DualSrcId::INTERAL0, DualSrcId::INTERNAL1, DualSrcId::INTERAL2});
        default: return {};
      }
    default: return {};
  }
...
static std::optional<DualSrcLayout> get_dual_op2_src_layout(uint8_t op1_count, uint8_t op2_count,
                                                            Imm2 src_config)
{
  switch(op1_count)
  {
      ...
      switch(op2_count)
      {
        case 1:
          switch(src_config)
          {
            case 0:
              return std::optional<DualSrcLayout>(
                  {DualSrcId::INTERAL0, DualSrcId::NONE, DualSrcId::NONE});
            case 1:
            case 2:
            case 3:
              return std::optional<DualSrcLayout>(
                  {DualSrcId::UNIFIED, DualSrcId::NONE, DualSrcId::NONE});
            default: return {};
          }
        case 2:
          switch(src_config)
        ...
        case 3:
          switch(src_config)
          {
            ...
            case 3:
              return std::optional<DualSrcLayout>(
                  {DualSrcId::INTERAL0, DualSrcId::UNIFIED, DualSrcId::INTERNAL1});
            default: return {};
          }
        default: return {};
      }
      ...
      switch(op2_count)
      {
        ...
        case 2:
          switch(src_config)
          {
            case 0:
              return std::optional<DualSrcLayout>(
                  {DualSrcId::INTERAL0, DualSrcId::INTERAL2, DualSrcId::NONE});
            case 1:
              return std::optional<DualSrcLayout>(
                  {DualSrcId::INTERNAL1, DualSrcId::INTERAL2, DualSrcId::NONE});
            case 2:
              return std::optional<DualSrcLayout>(
                  {DualSrcId::INTERAL2, DualSrcId::UNIFIED, DualSrcId::NONE});
            default: return {};
          }
        default: return {};
      }
      ...
      switch(op2_count)
      {
        case 1:
          switch(src_config)
          {
            ...
            case 2:
              return std::optional<DualSrcLayout>(
                  {DualSrcId::INTERAL2, DualSrcId::NONE, DualSrcId::NONE});
            case 3:
              return std::optional<DualSrcLayout>(
                  {DualSrcId::UNIFIED, DualSrcId::NONE, DualSrcId::NONE});
            default: return {};
...
bool USSETranslatorVisitor::vdual(
    Imm1 comp_count_type, Imm1 gpi1_neg, Imm2 sv_pred, Imm1 skipinv,
    Imm1 dual_op1_ext_vec3_or_has_w_vec4, bool type_f16, Imm1 gpi1_swizz_ext,
    Imm4 unified_store_swizz, Imm1 unified_store_neg, Imm3 dual_op1, Imm1 dual_op2_ext,
    bool prim_ustore, Imm4 gpi0_swizz, Imm4 gpi1_swizz, Imm2 prim_dest_bank,
    Imm2 unified_store_slot_bank, Imm2 prim_dest_num_gpi_case, Imm7 prim_dest_num, Imm3 dual_op2,
    Imm2 src_config, Imm1 gpi2_slot_num_bit_1, Imm1 gpi2_slot_num_bit_0_or_unified_store_abs,
    Imm2 gpi1_slot_num, Imm2 gpi0_slot_num, Imm3 write_mask_non_gpi, Imm7 unified_store_slot_num)
{
  Instruction op1;
  Instruction op2;

  const Opcode op1_codes[16] = {
      Opcode::VMAD, Opcode::VDP,  Opcode::VSSQ,    Opcode::VMUL,
      Opcode::VADD, Opcode::VMOV, Opcode::FRSQ,    Opcode::FRCP,

      Opcode::FMAD, Opcode::FADD, Opcode::FMUL,    Opcode::FSUBFLR,
      Opcode::FEXP, Opcode::FLOG, Opcode::INVALID, Opcode::INVALID,
  };

  const Opcode op2_codes[16] = {
      Opcode::INVALID, Opcode::VDP,  Opcode::VSSQ,    Opcode::VMUL,
      Opcode::VADD,    Opcode::VMOV, Opcode::FRSQ,    Opcode::FRCP,
  ...
  const std::map<Opcode, DualOpInfo> op_info = {
      {Opcode::VMAD, {3, true, true}},   {Opcode::VDP, {2, true, false}},
      {Opcode::VSSQ, {1, true, false}},  {Opcode::VMUL, {2, true, true}},
      {Opcode::VADD, {2, true, true}},   {Opcode::VMOV, {1, true, true}},
      {Opcode::FRSQ, {1, false, false}}, {Opcode::FRCP, {1, false, false}},
      {Opcode::FMAD, {3, false, false}}, {Opcode::FADD, {2, false, false}},
      {Opcode::FMUL, {2, false, false}}, {Opcode::FSUBFLR, {2, false, false}},
      {Opcode::FEXP, {1, false, false}}, {Opcode::FLOG, {1, false, false}},
  };
  ...
  unified_dest =
      decode_dest(unified_dest, prim_dest_num, prim_dest_bank, false, true, 8, m_second_program);
  ...
  const std::optional<DualSrcLayout> op1_layout =
      get_dual_op1_src_layout(op1_info.src_count, src_config);
  const std::optional<DualSrcLayout> op2_layout =
      get_dual_op2_src_layout(op1_info.src_count, op2_info.src_count, src_config);

  if(!op1_layout)
  {
    RDCERR("Missing dual for op1 layout.");
    return false;
  }

  if(!op2_layout)
  {
    RDCERR("Missing dual for op2 layout.");
    return false;
  }

  Imm4 fixed_write_mask =
      write_mask_non_gpi | ((comp_count_type && dual_op1_ext_vec3_or_has_w_vec4) << 3u);

  Imm4 op1_write_mask =
      prim_ustore ? get_dual_op_write_mask(op1_info, op1.opr.dest.bank == RegisterBank::FPINTERNAL)
                  : fixed_write_mask;
  Imm4 op2_write_mask =
      prim_ustore ? fixed_write_mask
                  : get_dual_op_write_mask(op2_info, op2.opr.dest.bank == RegisterBank::FPINTERNAL);
      ...
      switch(id)
      {
        case DualSrcId::UNIFIED:
          ...
          op.swizzle = decode_dual_swizzle(unified_store_swizz,
                                           code_info.src_count >= 2 ? false : gpi2_slot_num_bit_1,
                                           comp_count_type);
        ...
        case DualSrcId::INTERAL2:
          ...
          op.num = code_info.src_count >= 2
                       ? (gpi2_slot_num_bit_1 << 1u | gpi2_slot_num_bit_0_or_unified_store_abs)
                       : 2;
  ...
  auto do_dual_op = [&](Opcode code, std::vector<Operand> &ops, Operand &dest, Imm4 write_mask_dest,
                        const DualOpInfo &code_info) {
    uint32_t write_mask_source =
        code_info.vector_load ? (0b1111u >> (uint32_t)!comp_count_type) : 0b0001;
    ...
    switch(code)
    {
      ...
      {
        const spv::Id source = load(ops[0], write_mask_source);
        result = m_b.createBuiltinCall(m_b.getTypeId(source), std_builtins, GLSLstd450InverseSqrt,
                                       {source});
        break;
      }
      case Opcode::FMUL:
      ...
      default:
        RDCERR("Missing implementation for DUAL %s.", disasm::opcode_str(code).c_str());
        return spv::NoResult;
    }

    disasm_str += disasm::opcode_str(code) + " " + disasm::operand_to_str(dest, write_mask_dest);
