...
bool USSETranslatorVisitor::vpck(
    ...
    if (inst.opr.src1.type == DataType::F32 && inst.opr.src2.bank != RegisterBank::IMMEDIATE) {
        ...
        if (!is_default(inst.opr.src1.swizzle, (shader::usse::Imm4)dest_mask_to_comp_count(dest_mask)) || !is_two_source_same || !is_contiguous) {
...
bool USSETranslatorVisitor::vldst(
    ...
    const int total_bytes_fo_fetch = (int)(get_data_type_size(type_to_ldst) * total_number_to_fetch);
    ...
    if (inst.opr.src1.bank == RegisterBank::IMMEDIATE) {
        inst.opr.src1.num *= (shader::usse::OperandNum)get_data_type_size(type_to_ldst);
    }
